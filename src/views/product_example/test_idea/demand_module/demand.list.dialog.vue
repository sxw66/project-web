          ...
          <div class="client-tabs">
            <el-tabs v-model="activeName" @tab-click="handleClick">
              <el-tab-pane label="基本信息" name="first">
                <div class="text-secondary"> 需求介绍 </div>
          ...
          <el-collapse v-model="activeNames">
            <el-collapse-item title="属性" name="1">
              <el-form-item label="标题" prop="demandTitle">
                <el-input v-model="state.demandTitle" />
              </el-form-item>
              <el-form-item label="需求类型" prop="demandType">
                <el-select v-model="state.demandType" style="width: 100%">
                  <el-option
              ...
              <el-form-item label="进度">
                <el-input type="number" v-model="state.deSchedule" />
              </el-form-item>
            </el-collapse-item>
...
<script setup lang="ts">
  ...
  import { delCustomerPages, updateDemandList } from '@/api/modules/product'
  // 表格列表-等级
  import { operationOptions } from '@/utils/public-data'
  import { selectPriorityList, demandTypeList, fromTypeList } from './static'
  // 异步加载富文本选择器
  ...
  const handleUpdateCustomerPages = async (state) => {
    const { code, data } = await updateDemandList(state)
    if (code === 200 && data) {
      ElMessage({
        type: 'success',
        message: '更新成功',
  ...
  watchEffect(() => {
    ...
    if (props.ListFormData) {
      ...
      state.demandTitle = props.ListFormData.demandTitle
      state.demandType = props.ListFormData.demandType
      ...
      state.deSchedule = props.ListFormData.deSchedule
