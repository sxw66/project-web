<template>
  <el-popover :visible="popoverShow" trigger="click" width="650px" placement="bottom-start">
    <template #reference>
      <el-button text @click="handleClickFilter">
  ...
  <div class="flex-align-center">
    <el-dropdown @command="handleCommand" trigger="click">
      <el-button text>
        <svg-icon :name="headerStatus.icons"></svg-icon>
        <span style="margin-left: 5px">{{ headerStatus.label }}</span>
      </el-button>
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item
            v-for="(item, index) in tableHeadMenu"
            :key="index"
            :command="item.command"
          >
            <svg-icon :name="item.icon" :color="item.color"></svg-icon>
...
<script setup lang="ts">
  ...
  import { Fold, Operation, Plus } from '@element-plus/icons-vue'
  // 接收父组件传递的参数
  ...
  const emit = defineEmits(['handleFilterChange', 'formLayout'])
  const popoverShow = ref(false) // 筛选的弹框状态
  // 表头切换的状态
  const headerStatus: any = reactive({
    icons: 'company',
    label: '树状',
  })
  ...
  // 当前表格结构状态
  const tableHeadMenu = computed(() => {
    return [
      {
        command: 'arborescence',
        label: `树状`,
        icon: 'company',
        color: '#999',
      },
  ...
  const handleCommand = (command: string | number | object) => {
    console.log('======', command)
    if (command === 'tiled') {
      headerStatus.icons = 'all'
      headerStatus.label = '平铺'
    }
    if (command === 'arborescence') {
      headerStatus.icons = 'company'
      headerStatus.label = '树状'
    }
    emit('formLayout', headerStatus.label)
  }
