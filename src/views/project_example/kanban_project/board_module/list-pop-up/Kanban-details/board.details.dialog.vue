<template>
  <!-- 看板设置-看板详情-弹框 -->
  <el-dialog
    v-model="props.boardDetailsVisible"
    v-if="props.boardDetailsVisible"
    title="全部工单"
    ...
    <template #header="{ close }">
      <div class="my-header flex-justify-between">
        ...
        <el-tooltip placement="top">
          ...
          <DropdownMenu :items="chilrenMoreMenu" @command="handleCommand">
...
<script setup lang="ts">
  ...
  const props = defineProps({
    boardDetailsVisible: Boolean,
    ListFormData: Object,
  })

  //分发事件给父组件
  let emits = defineEmits(['closeBoardDetailsDialog'])
  ...
  // Nav列表子孙菜单
  const workOrderTypeList = computed(() => {
    return [
      {
        label: '需求',
        value: 0,
  ...
  // 关闭-取消
  const handleCancel = () => {
    emits('closeBoardDetailsDialog', state.IsModify)
  }
  ...
  const handleconfirm = async () => {
    ...
    params.principal =
      Array.isArray(params.principal) && params.principal.length > 0
        ...
        : ''
    emits('closeBoardDetailsDialog', true)
  }
